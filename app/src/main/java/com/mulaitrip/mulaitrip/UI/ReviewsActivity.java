...
//    private static String URL = "http://mulaitrip.com/api/reviews/places/create/";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        placeid = String.valueOf((this.getIntent().getStringExtra("id")));
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!TextUtils.isEmpty(rate) && !TextUtils.isEmpty(review)) {
                    sendReview(userid, placeid, rate, review);
                }
            }
        });
    }

    private void sendReview(String userid, String placeid, String rate, String review) {
...
//        ApiInterface apiInterface = retrofit.create(ApiInterface.class);
        ItemReview itemReview= new ItemReview(userid, placeid, rate, review);
        ApiInterface apiInterface = ApiClient.getApiClient().create(ApiInterface.class);
        Call<ItemReview> call = apiInterface.placeReview(itemReview, token);
        call.enqueue(new Callback<ItemReview>() {
            @Override
            public void onResponse(Call<ItemReview> call, Response<ItemReview> response) {
                if(response.isSuccessful()) {
                    Toast.makeText(ReviewsActivity.this, "Review submitted" + response.body(), Toast.LENGTH_LONG).show();
                    movetoMain(token);
                } else {
                    if (response.errorBody().contentType().subtype().equals("json")) {
                        ApiError apiError = ApiError.fromResponseBody(response.errorBody());
                        Toast.makeText(ReviewsActivity.this, apiError.getMessage(), Toast.LENGTH_LONG).show();
