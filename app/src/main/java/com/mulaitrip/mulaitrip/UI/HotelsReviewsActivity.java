...
public class HotelsReviewsActivity extends AppCompatActivity {
    private static final String TAG = HotelsReviewsActivity.class.getSimpleName();
...
//    private static String URL = "http://mulaitrip.com/api/reviews/places/create/";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        hotelid = String.valueOf((this.getIntent().getStringExtra("id")));
        ...
        btnSubmit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(!TextUtils.isEmpty(rate) && !TextUtils.isEmpty(review)) {
                    sendReview(userid, hotelid, rate, review);
                }
            }
        });
    }

    private void sendReview(String userid, String hotelid, String rate, String review) {
...
//        ApiInterface apiInterface = retrofit.create(ApiInterface.class);
        HotelReview hotelReview= new HotelReview(userid, hotelid, rate, review);
        ApiInterface apiInterface = ApiClient.getApiClient().create(ApiInterface.class);
        Call<HotelReview> call = apiInterface.hotelReview(hotelReview, token);
        call.enqueue(new Callback<HotelReview>() {
            @Override
            public void onResponse(Call<HotelReview> call, Response<HotelReview> response) {
                if(response.isSuccessful()) {
                    Toast.makeText(HotelsReviewsActivity.this, "Review submitted" + response.body(), Toast.LENGTH_LONG).show();
                    movetoMain(token);
                } else {
                    if (response.errorBody().contentType().subtype().equals("json")) {
                        ApiError apiError = ApiError.fromResponseBody(response.errorBody());
                        Toast.makeText(HotelsReviewsActivity.this, apiError.getMessage(), Toast.LENGTH_LONG).show();
            ...
            @Override
            public void onFailure(Call<HotelReview> call, Throwable t) {
